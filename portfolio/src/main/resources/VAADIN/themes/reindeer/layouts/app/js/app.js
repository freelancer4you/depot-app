...
function addDepotListEntry(newLinkName) {
  var ul = document.getElementById("depotList");
  var li = document.createElement("li");
  
  var aTag = document.createElement('a');
  aTag.setAttribute('href',"#!depotView/" + newLinkName);
  aTag.innerHTML = newLinkName;
  
  li.appendChild(aTag);
  ul.appendChild(li);
}

function addRow(row, entry) {
	...
	// 0
	cell0.innerHTML = entry.position;
	
	// 1
	var percentageValueDiv = document.createElement("div");
	percentageValueDiv.setAttribute("id", entry.isin + "-td");
	percentageValueDiv.innerHTML = entry.percentage;	
	percentageValueDiv.onchange = function()
    {

//		console.debug("Wert für '" + percentageValueDiv.id +"' wurde geändert auf '" + percentageValueDiv.innerHTML +"'");
		
		// Spalte geplanter Anteil
		aktualisiereTableAuswertungGesamt();
		//aktualisiereTableAuswertungDepot();
		
		// todoTotal: Todo RisikoFrei + TodoRisiko
//		document.getElementById("todoTotal").innerHTML = percentageValueDiv.innerHTML;
    };
	cell1.appendChild(percentageValueDiv);
	...
	btn.setAttribute("onclick", "javascript:de.goldmann.portfolio.ui.rebalancing.editPosition('"+entry.isin+"');");
	var t = document.createTextNode("Edit");	
	btn.appendChild(t);	
	cell3.appendChild(btn);	
}

function aktualisiereTableAuswertungGesamt() {
	aktualisiereSpalteGeplanterAnteil();
	aktualisiereSpalteAtkuellerWert();
	aktualisiereSpalteAktuellerAnteil();
	aktualisiereSpalteGeplanterWert();
	aktualisiereSpalteTodo();
}

function aktualisiereSpalteGeplanterAnteil() {
	var cashPercentage = parseFloat(document.getElementById("cashPercentage").innerHTML);
	document.getElementById("c14").innerHTML = cashPercentage + "%";
	var percentageWithRisk = 100 - cashPercentage;
	document.getElementById("c15").innerHTML = percentageWithRisk + "%";
	var c16 = percentageWithRisk + cashPercentage;
	document.getElementById("c16").innerHTML = c16 + "%";
}

function aktualisiereSpalteAtkuellerWert() {
	var cashAmount = parseFloat(document.getElementById("cashAmount").innerHTML);
	document.getElementById("d14").innerHTML = cashAmount;
	var stocksAmount = parseFloat(document.getElementById("stocksAmount").innerHTML); 
	var etfAmount = 0.0;
	
	var depotAuswertungTable = document.getElementById("depotAuswertungTable");	
	var table = document.getElementById("positionsTable");
	
	var depotAuswertungTableZeilenEinfuegen =  depotAuswertungTable.rows.length == 1;
	console.debug("Zeilen:" + depotAuswertungTable.rows.length);
	for (var i = 0, row; row = table.rows[i]; i++) {
	   
		if(depotAuswertungTableZeilenEinfuegen) {
			var neueZeile = null;
			if(i > 1 && i < table.rows.length - 2) {
				neueZeile = depotAuswertungTable.insertRow(1);
			}	 
		}
		
	   for (var j = 0, col; col = row.cells[j]; j++) {
	     
		   if(j == 2 && i > 1 && i < table.rows.length - 2){
			   etfAmount += parseFloat(col.innerHTML);
			   if(depotAuswertungTableZeilenEinfuegen) {
				   var cell0 = neueZeile.insertCell(0);
			   	   var cell1 = neueZeile.insertCell(1);
			   	   var cell2 = neueZeile.insertCell(2);
			   	   var cell3 = neueZeile.insertCell(3);
			   	   var cell4 = neueZeile.insertCell(4);
			   	   var cell5 = neueZeile.insertCell(5);
			   	   var cell6 = neueZeile.insertCell(6);
			   }
		   }
	   }  
	}
	document.getElementById("d15").innerHTML = stocksAmount + etfAmount;
	document.getElementById("d16").innerHTML = stocksAmount + etfAmount + cashAmount;
	
}

function aktualisiereSpalteAktuellerAnteil() {
	
	var d14 = parseFloat(document.getElementById("d14").innerHTML);
	var d15 = parseFloat(document.getElementById("d15").innerHTML);
	var d15 = parseFloat(document.getElementById("d15").innerHTML);
	var amountSum = d14 + d15;
	var e14 = d14/(amountSum);
	var e15 = d15/(amountSum);
	document.getElementById("e14").innerHTML = e14 + "%";
	document.getElementById("e15").innerHTML = e15 + "%";
	document.getElementById("e16").innerHTML = (e14 + e15) + "%";
}

function aktualisiereSpalteGeplanterWert() {
	var d14 = parseFloat(document.getElementById("d14").innerHTML);
	var d15 = parseFloat(document.getElementById("d15").innerHTML);
	var d16 = parseFloat(document.getElementById("d16").innerHTML);
	var c15 = parseFloat(document.getElementById("c15").innerHTML);
	var c14 = parseFloat(document.getElementById("c14").innerHTML);
	var f15 = parseFloat(document.getElementById("f15").innerHTML);
	if(isNaN(f15)) {
		f15 = 0;
	}

	var f14 = d16*c14 < d14 ? d16*c14 : f15/c15*c14;
	var f15 = d16*c14 < d14 ? d14-d16*c14+d15 : d15;
	
	document.getElementById("f14").innerHTML = f14;  
	document.getElementById("f15").innerHTML = f15;
	document.getElementById("f16").innerHTML = f14 + f15;
}

function aktualisiereSpalteTodo() {
	var c14 = parseFloat(document.getElementById("c14").innerHTML);
	var d14 = parseFloat(document.getElementById("d14").innerHTML);
	var d15 = parseFloat(document.getElementById("d15").innerHTML);
	var d16 = parseFloat(document.getElementById("d16").innerHTML);
	var f16 = parseFloat(document.getElementById("f16").innerHTML);
	
	var g14 = f16*c14-d14;
	var g15 = (d16*c14<d14 ? d14-d16*c14+d15 :d15)-d15;
	
	document.getElementById("g14").innerHTML = g14;
	document.getElementById("g15").innerHTML = g15;
	document.getElementById("g16").innerHTML = g14 + g15;	
}

function aktualisiereTableAuswertungDepot() {
